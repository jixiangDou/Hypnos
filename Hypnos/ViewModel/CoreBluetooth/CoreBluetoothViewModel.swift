...
class CoreBluetoothViewModel: NSObject, ObservableObject, CBPeripheralProtocolDelegate, CBCentralManagerProtocolDelegate {
    ...
    func writeValue(value: String) {
        for c in foundCharacteristics {
            if c.uuid == CBUUID(string: "FFE1") {
                self.connectedPeripheral.peripheral.writeValue(Data(value.utf8), for: c.characteristic, type: .withResponse )
            }
        }
    }
